...
<template>
  <UContainer>
    <div class="flex items-center justify-between mb-6">
      ...
      <ButtonUButton label="View All" variant="link" color="primary" to="/products" />
    </div>
    <div class="grid grid-cols-5 gap-8">
      <div v-for="product in topProducts" :key="product.id" class="relative">
        <UBadge v-if="product.discount" color="error" class="absolute top-2 right-2 z-10">
          -{{ product.discount }}%
        </UBadge>
        ...
        <UCard :ui="{
          ...
          <!-- Product image -->
          <template #header>
            <NuxtImg :src="product.image" class="w-full aspect-square object-cover rounded-lg" />
          </template>

          <div class="flex flex-col content-around">
            <div class="flex items-center gap-1">
              <!-- v-for se 5 stars loop -->
              <span v-for="i in 5" :key="i" class="relative text-xl text-gray-300">
                ★
                <!-- Full yellow overlay -->
                <span v-if="i <= Math.floor(product.rating.rate)" class="absolute inset-0 text-yellow-400">★</span>
                <!-- Half yellow overlay -->
                <span v-else-if="
                  i === Math.floor(product.rating.rate) + 1 &&
                  product.rating.rate % 1 >= 0.5
                " class="absolute inset-0 text-yellow-400 overflow-hidden" style="width: 50%">★</span>
              </span>
